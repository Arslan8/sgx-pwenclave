...
int test(void)
{
  ...
  ret = pw_setup(eid, &pwerr, password, strlen((const char*)password), blob, sizeof blob, &bloblen);
  if (ret != SGX_SUCCESS)
  {
  ...
  /* -- Check it works -- */
  ret = pw_check(eid, &pwerr, password, strlen((const char*)password), blob, bloblen);
  if (ret != SGX_SUCCESS)
  {
  ...
  ret = pw_check(eid, &pwerr, wrong_password, strlen((const char*)wrong_password), blob, bloblen);
  if (ret != SGX_SUCCESS)
  {
    printf("sgx pw_check failed: %#x\n", ret);
    sgx_destroy_enclave(eid);
